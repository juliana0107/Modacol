...
@section('title', 'Crear Flujo de Caja')

@section('content')

<h1>Crear Flujo de Caja</h1>
...
<form action="{{ route('flujoCaja.store') }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <input type="number" step="0.01" class="form-control @error('Saldo_Neto') is-invalid @enderror" id="Saldo_Neto" name="Saldo_Neto" value="{{ old('Saldo_Neto') }}">
        @error('Saldo_Neto')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="mb-3">
        ...
        <input type="number" step="0.01" class="form-control @error('Saldo_Final') is-invalid @enderror" id="Saldo_Final" name="Saldo_Final" value="{{ old('Saldo_Final') }}">
        @error('Saldo_Final')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
